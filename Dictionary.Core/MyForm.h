...
#pragma region For Windows Form Designer, modify too much = no designer editor
		void InitializeComponent(void)
		{
			...
			this->skip_run->Click += gcnew System::EventHandler(this, &MyForm::skip_run_Click);
			// 
			// add_time
			// 
			...
			this->setting_addfile_run->Click += gcnew System::EventHandler(this, &MyForm::setting_addfile_run_Click);
			// 
			// setting_browse_run
			// 
			...
			this->setting_browse_run->Click += gcnew System::EventHandler(this, &MyForm::setting_browse_run_Click);
			// 
			// label8
			// 
...
#pragma region Init
		void Init()
		{
			...
			this->parse_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Parse);

			this->sort_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Sort);

			this->search_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Search);
			this->search_box->KeyUp += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::SearchBoxOnEnterPressed);
			this->search_box->KeyDown += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::SupressEnter);

			this->add_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Add);
			this->add_definition->KeyUp += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::DefinitionBoxOnEnterPressed);
			this->add_definition->KeyDown += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::SupressEnter);

			this->view_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Display);

			this->validate_run->Click += gcnew System::EventHandler(this, &MyForm::Run_Validate);
			this->validate_box->KeyUp += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::ValidateBoxOnEnterPressed);
			this->validate_box->KeyDown += gcnew System::Windows::Forms::KeyEventHandler(this, &MyForm::SupressEnter);
		}
		void InitDSItem(ToolStripMenuItem^ item, String^ text)
		{
			item->Text = text;
			item->Click += gcnew System::EventHandler(this, &MyForm::DS_SelectionChanged);
...
#pragma region delegates
		...
		//delegate void ProgressBarUpdater(int value, ProgressBar^ progressbar); 
...
#pragma region Control Updaters
		...
		void UpdateTrigger(String^ Text, bool IsEnabled, Label^ label)
		{
			...
			{  
				LabelUpdater^ d = gcnew LabelUpdater(this, &MyForm::UpdateTrigger);  
		...
		{   
			...
			{  
				TimerUpdater^ d = gcnew TimerUpdater(this, &MyForm::UpdateTimer);  
		...
		{   
			...
			{  
				RichTextBoxUpdater^ d = gcnew RichTextBoxUpdater(this, &MyForm::UpdateRichTextBox);  
		...
		{   
			if (box->InvokeRequired)  
			{  
				TextBoxUpdater^ d = gcnew TextBoxUpdater(this, &MyForm::UpdateTextBox);  
				this->Invoke(d, gcnew array<Object^> {box,text });  
			}  
			else   
				box->Text = text;  
		...
		{   
			...
			{  
				TabIndexUpdater^ d = gcnew TabIndexUpdater(this, &MyForm::SetTabIndex);  
...
#pragma region Common Checks
		...
		bool IsOperationInProgress()
		{
			...
			{
				Ut::Error("Another Operation In Progress.");
				return true;
			}
			return false;
		}
		bool IsLinkedListLoaded()
		{
			if(!list->IsLoaded)
			{
				Ut::Error("LinkedList Not Loaded");
		...
		bool CheckUp()
		{
			...
			{
				if(!IsLinkedListLoaded())
					return false;
			}
			else
			{
				//Todo: Check if BinaryTree Loaded
				Ut::Error("Not Implemented");
				return false;
			}
			return true;
		}
...
#pragma region Timer Thread Handler
		void TimerThread(Object^ label)
		{
			...
			while(true)
			{
				//Time Now - Last Time//
				EnTime=clock()-StTime;
				//Update Text of referenced timer Lable to calculated time//
...
#pragma region Parse Dictionary
		Void Run_Parse(Object^  sender, System::EventArgs^  e) {
			...
			Thread^ thread = gcnew Thread( gcnew ThreadStart( this, &MyForm::ParseThread ) );
			thread->Start();
		}
		...
		void ParseThread() {
			...
			Thread^ thread3 = gcnew Thread(gcnew ParameterizedThreadStart(this, &MyForm::TimerThread));
...
#pragma region Sort Dictionary
		Void Run_Sort(Object^  sender, System::EventArgs^  e) {
			...
			Thread^ thread = gcnew Thread( gcnew ThreadStart( this, &MyForm::SortThread ) );
			thread->Start();
...
#pragma region Search Dictionary
		...
		void RunSearch(String^ wordname)
		{
			...
			Thread^ thread = gcnew Thread( gcnew ParameterizedThreadStart( this, &MyForm::SearchThread ) );
		...
		void SearchThread(Object^ wordname) { 
			...
			if(LinkedListItem->Checked)
				...
				//Todo: Search Binary Tree, Return ResultSet
			}
			thread3->Abort();
...
#pragma region Add To Dictionary
		...
		void RunAdd()
		{
			if(!CheckUp())
				return;
			Thread^ thread = gcnew Thread( gcnew ParameterizedThreadStart( this, &MyForm::AddThread ) );
		...
		void AddThread(Object^ word) { 
			...
			if(LinkedListItem->Checked)
				...
				//Todo: Add Word To Binary Tree, Return ResultSet
			}
			thread3->Abort();
			OperationInProgress = false;
			if(result.result.GetName() == "")
			{
				...
				SetTabIndex(2);
				RunSearch(Ut::ToStringHat(addword->word->GetName().c_str()));
			}
		}
...
#pragma region Display All In Dictionary
		...
		void RunDisplay()
		{
			if(!CheckUp())
				return;
			Thread^ thread = gcnew Thread( gcnew ThreadStart( this, &MyForm::DisplayThread ) );
			AddWord^ ToAdd = gcnew AddWord();
			thread->Start();
		}
		void DisplayThread() { 
			...
			if(LinkedListItem->Checked)
				...
				//Todo: Get All Word From Binary Tree
			}
			thread3->Abort();
...
#pragma region Validate Sentence
		...
		void RunValidate()
		{
			if(!CheckUp())
				return;
			Thread^ thread = gcnew Thread(gcnew ThreadStart(this, &MyForm::ValidateThread ) );
			thread->Start();
		}
		void ValidateThread() { 
			...
			{
				//Todo: Validate Sentense Through Binary Tree
			}
			thread3->Abort();
